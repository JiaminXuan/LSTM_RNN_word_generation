...
function query_sentences()
  while true do
    ...
    if not ok then
      ...
      --create input x, y y doesn't need any real input
      local x = transfer_data(torch.zeros(params.batch_size))
      local y = transfer_data(torch.ones(params.batch_size))
      --feed input word to model
      for i=2, #line do
        --get word
        word_to_predict = line[i]
        --get index of the word in dictionary
        local idx = ptb.vocab_map[word_to_predict]
        --fill batch with index
        x:fill(idx)
        perp_q, model.s[1], pred_q = unpack(model.rnns[1]:forward({x, y, model.s[0]}))
      ...
      for i = 1,len do
        ...
        idx = ptb.vocab_map[word_to_predict]
        x:fill(idx)
        --get prediction from model
        perp_q, model.s[1], pred_q = unpack(model.rnns[1]:forward({x, y, model.s[0]}))
        --get prediction to cpu
        next_idx = pred_q[1]:clone():float()
        --get nultinomial result
        next_idx = torch.multinomial(torch.exp(next_idx),1)
        --set next word to predicted word
        next_word=ptb.vocab_map_inverse[next_idx[1]]
        io.write(next_word,' ')
        io.flush()
        g_replace_table(model.s[0], model.s[1])
      end
...
function bp(state)
  ...
  for i = params.seq_length, 1, -1 do
    ...
    local depred = transfer_data(torch.zeros(params.batch_size,params.vocab_size))
    local tmp = model.rnns[i]:backward({x, y, s},
                                       {derr, model.ds,depred})[3]
...
function run_test()
  ...
  for i = 1, (len - 1) do
    ...
    local s = model.s[i - 1]
    perp_q, model.s[1] = unpack(model.rnns[1]:forward({x, y, model.s[0]}))
    perp = perp + perp_q[1]
    g_replace_table(model.s[0], model.s[1])
  end
...
model_save.params = params
torch.save("model.rnn",model_save)
